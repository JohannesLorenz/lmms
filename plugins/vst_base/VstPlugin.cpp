...
bool VstPlugin::processMessage(const message& _m) {
	...
	case IdVstPluginWindowID:
		...
		if (m_embedMethod == "none" && ConfigManager::inst()->value("ui", "vstalwaysontop").toInt()) {
			...
			SetWindowLongPtr(
				(HWND)(intptr_t)m_pluginWindowID, GWLP_HWNDPARENT, (LONG_PTR)getGUI()->mainWindow()->winId());
#endif

#ifdef LMMS_BUILD_LINUX
			XSetTransientForHint(QX11Info::display(), m_pluginWindowID, getGUI()->mainWindow()->winId());
#endif
		}
		break;
...
void VstPlugin::savePreset() {
	...
	FileDialog sfd(
		nullptr, tr("Save Preset"), presName.section(": ", 1, 1) + tr(".fxp"), tr("Vst Plugin Preset (*.fxp *.fxb)"));
