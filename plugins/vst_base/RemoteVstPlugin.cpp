...
public:
	...
	virtual void updateSampleRate() {
		...
		pluginDispatch(effSetSampleRate, 0, 0, nullptr, (float)sampleRate());
	}
...
void RemoteVstPlugin::initEditor() {
	...
	if (m_display == nullptr) { m_display = XOpenDisplay(nullptr); }
	...
	XSelectInput(m_display, m_window,
		SubstructureNotifyMask | ButtonPressMask | ButtonReleaseMask | ButtonMotionMask | ExposureMask | KeyPressMask);
...
bool RemoteVstPlugin::load(const std::string& _plugin_file) {
	...
	if (mainEntry == nullptr) { mainEntry = (mainEntryPointer)GetProcAddress(m_libInst, "VstPluginMain"); }
	if (mainEntry == nullptr) { mainEntry = (mainEntryPointer)GetProcAddress(m_libInst, "main"); }
#else
	mainEntryPointer mainEntry = (mainEntryPointer)dlsym(m_libInst, "VSTPluginMain");
	if (mainEntry == nullptr) mainEntry = (mainEntryPointer)dlsym(m_libInst, "VstPluginMain");
...
#ifdef NATIVE_LINUX_VST
void RemoteVstPlugin::sendX11Idle() { queueMessage(message(IdIdle)); }
#endif
...
#endif
{
	...
	return 0;
#else
	if (m.id == IdQuit) { _this->queueMessage(m); }
...
bool RemoteVstPlugin::setupMessageWindow() {
	...
	__MessageHwnd = CreateWindowEx(0, "LVSL", "dummy", 0, 0, 0, 0, 0, nullptr, nullptr, hInst, nullptr);
...
void RemoteVstPlugin::guiEventLoop() {
	...
	while (true) {
		...
		if (__plugin->isInitialized()) { __plugin->idle(); }

		if (__plugin->isInitialized() && !__plugin->isProcessing()) { __plugin->processUIThreadMessages(); }
...
#ifdef NATIVE_LINUX_VST
	if (EMBED) { cerr << "Native linux VST works only without embedding." << endl; }
#endif
