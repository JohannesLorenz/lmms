...
void vestigeInstrument::loadSettings(const QDomElement& _this)
{
	...
	if (m_plugin != nullptr)
	{
		...
		if (instrumentTrack() != nullptr && instrumentTrack()->isPreviewMode()) { m_plugin->hideUI(); }
...
void vestigeInstrument::setParameter(Model* action)
{
	...
	if (m_plugin != nullptr) { m_plugin->setParam(knobUNID, knobFModel[knobUNID]->value()); }
}
...
{
	m_pluginMutex.lock();
	const bool set_ch_name = (m_plugin != nullptr && instrumentTrack()->name() == m_plugin->name())
	...
	if (m_plugin != nullptr) { closePlugin(); }
	m_pluginDLL = PathUtil::toShortestRelative(_file);
...
{
	m_pluginMutex.lock();
	if (m_plugin != nullptr) { m_plugin->processMidiEvent(event, offset); }
	m_pluginMutex.unlock();
...
void vestigeInstrument::closePlugin(void)
{
	...
	if (m_subWindow != nullptr)
	{
		...
		if (m_subWindow != nullptr) { delete m_subWindow; }
		m_subWindow = nullptr;
	}

	if (p_subWindow != nullptr) { p_subWindow = nullptr; }
...
{
	if (m_vi == nullptr || m_vi->m_plugin == nullptr) { return; }
...
void VestigeInstrumentView::paintEvent(QPaintEvent*)
{
	...
	QString plugin_name = (m_vi->m_plugin != nullptr) ? m_vi->m_plugin->name() /* + QString::number(
																 m_plugin->version() )*/
	...
	: InstrumentViewFixedSize(_instrument, _parent)
{
	...
	m_vi->m_subWindow = getGUI()->mainWindow()->addWindowedWidget(
		nullptr, Qt::SubWindow | Qt::CustomizeWindowHint | Qt::WindowTitleHint | Qt::WindowSystemMenuHint);
...
manageVestigeInstrumentView::~manageVestigeInstrumentView()
{
	...
	if (m_vi->m_subWindow != nullptr)
	{
		...
		if (m_vi->m_subWindow != nullptr) delete m_vi->m_subWindow;
