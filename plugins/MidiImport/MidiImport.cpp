...
bool MidiImport::tryImport(TrackContainer* tc)
{
	...
	if (getGUI() != nullptr && ConfigManager::inst()->sf2File().isEmpty())
	{
		QMessageBox::information(getGUI()->mainWindow(), tr("Setup incomplete"),
...
bool MidiImport::readSMF(TrackContainer* tc)
{
	...
	AutomationClip* timeSigNumeratorPat = new AutomationClip(nt);
	timeSigNumeratorPat->setDisplayName(tr("Numerator"));
	timeSigNumeratorPat->addObject(&timeSigMM.numeratorModel());
	AutomationClip* timeSigDenominatorPat = new AutomationClip(dt);
	...
	for (int t = 0; t < seq->tracks(); ++t)
	{
		...
		for (int e = 0; e < trk->length(); ++e)
		{
			...
			else if (evt->is_note())
			{
				...
				Note n((ticks < 1 ? 1 : ticks), noteEvt->get_start_time() * ticksPerBeat, noteEvt->get_identifier(),
						...
						if (objModel)
						{
							...
							else
							{
								if (ccs[ccid].at == nullptr)
								{
									ccs[ccid].create(tc,
										trackName + " > "
											+ (objModel != nullptr ? objModel->displayName()
	...
	for (auto& c : chs)
	{
		if (c.second.hasNotes) { c.second.splitMidiClips(); }
		else if (c.second.it)
		{
