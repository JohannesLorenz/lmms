...
Plugin::Descriptor PLUGIN_EXPORT ladspaeffect_plugin_descriptor = {STRINGIFY(PLUGIN_NAME), "LADSPA",
	QT_TRANSLATE_NOOP("PluginBrowser",
		...
		"inside LMMS."),
	"Danny McRae <khjklujn/at/users.sourceforge.net>", 0x0100, Plugin::Effect, new PluginPixmapLoader("logo"), nullptr,
...
bool LadspaEffect::processAudioBuffer(sampleFrame* _buf, const fpp_t _frames)
{
	...
	if (m_maxSampleRate < Engine::audioEngine()->processingSampleRate()) {
		...
		frames = _frames * m_maxSampleRate / Engine::audioEngine()->processingSampleRate();
	}
	...
	if (o_buf != nullptr) { sampleBack(_buf, o_buf, m_maxSampleRate); }
