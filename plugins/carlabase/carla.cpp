	...
	, m_paramModels()
{
	...
	if (fHandle != nullptr && fDescriptor->activate != nullptr) fDescriptor->activate(fHandle);
	...
	for (int i = 0; i < paramCount; ++i)
	{
		...
		connect(
			m_paramModels[i], &CarlaParamFloatModel::dataChanged, this, [this, i]() { paramModelChanged(i); },
			Qt::DirectConnection);
	}
#endif
...
CarlaInstrument::~CarlaInstrument()
{
	...
	if (fDescriptor->deactivate != nullptr) fDescriptor->deactivate(fHandle);

	if (fDescriptor->cleanup != nullptr) fDescriptor->cleanup(fHandle);
...
uint32_t CarlaInstrument::handleGetBufferSize() const { return Engine::audioEngine()->framesPerPeriod(); }
	...
	const intptr_t value, void* const ptr, const float opt)
{
	...
	case NATIVE_HOST_OPCODE_UI_TOUCH_PARAMETER:
		...
		// true = mousePress
		// false = mouseRelease
		if (!value) { updateParamModel(index); }
		break;
	case NATIVE_HOST_OPCODE_RELOAD_ALL: refreshParams(); break;
	case NATIVE_HOST_OPCODE_UPDATE_PARAMETER: break;
	case NATIVE_HOST_OPCODE_RELOAD_PARAMETERS: refreshParams(); break;
	case NATIVE_HOST_OPCODE_UPDATE_MIDI_PROGRAM:
	case NATIVE_HOST_OPCODE_RELOAD_MIDI_PROGRAMS:
	case NATIVE_HOST_OPCODE_INTERNAL_PLUGIN:
	case NATIVE_HOST_OPCODE_QUEUE_INLINE_DISPLAY: break;
#endif
	default: break;
	}
...
void CarlaInstrument::refreshParams(bool init)
{
		...
		&& fDescriptor->get_parameter_value != nullptr && fDescriptor->set_parameter_value != nullptr)
	{
		...
		for (uint32_t i = 0; i < paramCount; ++i)
		{
			...
			if (paramInfo->name != nullptr) { name = paramInfo->name; }
			...
			m_paramModels[i]->setRange(paramInfo->ranges.min, paramInfo->ranges.max, paramInfo->ranges.step);
	...
	, m_paramsView(nullptr)
{
	...
	m_toggleUIButton->setToolTip(tr("Click here to show or hide the graphical user interface (GUI) of Carla."));
...
void CarlaInstrumentView::toggleParamsWindow()
{
	...
	{
		if (m_paramsSubWindow->isVisible()) { m_paramsSubWindow->hide(); }
		else
		{
...
void CarlaInstrumentView::paramsUiClosed() { m_toggleParamsWindowButton->setChecked(false); }
	...
	, m_curOutRow(0)
{
	...
	m_automatedOnlyButton->setToolTip(tr("Only show knobs with a connection."));
	...
	// -- Sub window
	CarlaParamsSubWindow* win = new CarlaParamsSubWindow(getGUI()->mainWindow()->workspace()->viewport(),
...
CarlaParamsView::~CarlaParamsView()
{
	...
	// Clear models
	if (m_carlaInstrument->m_paramModels.isEmpty() == false) { m_carlaInstrument->clearParamModels(); }
}
...
void CarlaParamsView::filterKnobs()
{
	...
	for (uint32_t i = 0; i < m_knobs.count(); ++i)
	{
		...
		if (!m_carlaInstrument->m_paramModels[i]->enabled()) { continue; }

		// Filter on automation only
		if (m_automatedOnlyButton->isChecked())
		{
			if (!m_carlaInstrument->m_paramModels[i]->isAutomatedOrControlled()) { continue; }
		}

		// Filter on group name
		if (m_groupFilterCombo->currentText() != m_carlaInstrument->m_paramModels[i]->groupName()) { continue; }

		// Filter on text
		if (text != "")
		{
			if (m_knobs[i]->objectName().contains(text, Qt::CaseInsensitive)) { addKnob(i); }
		}
		else
		{
			addKnob(i);
...
void CarlaParamsView::refreshKnobs()
{
	...
	if (!groupNameList.count()) { groupNameList.append("No params"); }
...
void CarlaParamsView::addKnob(uint32_t index)
{
	...
	if (output)
	{
		...
		m_knobs[index]->show();
		if (m_curOutColumn < m_maxColumns - 1) { m_curOutColumn++; }
		else
		{
			m_curOutColumn = 0;
			m_curOutRow++;
	...
	else
	{
		...
		if (m_curColumn < m_maxColumns - 1) { m_curColumn++; }
		else
		{
			m_curColumn = 0;
			m_curRow++;
