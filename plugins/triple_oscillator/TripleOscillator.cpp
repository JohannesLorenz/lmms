	...
	, m_useWaveTable(true)
{
	...
	connect(&m_useWaveTableModel, SIGNAL(dataChanged()), this, SLOT(updateUseWaveTable()));
...
void OscillatorObject::updateUseWaveTable() { m_useWaveTable = m_useWaveTableModel.value(); }
...
void TripleOscillator::saveSettings(QDomDocument& _doc, QDomElement& _this)
{
	for (int i = 0; i < NUM_OF_OSCILLATORS; ++i)
	{
		...
		m_osc[i]->m_useWaveTableModel.saveSettings(_doc, _this, "useWaveTable" + QString::number(i + 1));
...
void TripleOscillator::loadSettings(const QDomElement& _this)
{
	for (int i = 0; i < NUM_OF_OSCILLATORS; ++i)
	{
		...
		m_osc[i]->m_useWaveTableModel.loadSettings(_this, "useWaveTable" + QString::number(i + 1));
	...
	: InstrumentViewFixedSize(_instrument, _parent)
{
	...
	for (int i = 0; i < NUM_OF_OSCILLATORS; ++i)
	{
		...
		uwt->move(110, btn_y);
		uwt->setActiveGraphic(PLUGIN_NAME::getIconPixmap("wavetable_active"));
		uwt->setInactiveGraphic(PLUGIN_NAME::getIconPixmap("wavetable_inactive"));
		...
		m_oscKnobs[i] = OscillatorKnobs(vk, pk, ck, flk, frk, pok, spdk, uwb, wsbg, uwt);
	}
}
...
void TripleOscillatorView::modelChanged()
{
	...
	for (int i = 0; i < NUM_OF_OSCILLATORS; ++i)
	{
		...
		m_oscKnobs[i].m_multiBandWaveTableButton->setModel(&t->m_osc[i]->m_useWaveTableModel);
