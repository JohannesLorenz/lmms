...
Plugin::Descriptor PLUGIN_EXPORT xpressive_plugin_descriptor
	= {STRINGIFY(PLUGIN_NAME), "Xpressive", QT_TRANSLATE_NOOP("PluginBrowser", "Mathematical expression parser"),
		"Orr Dvori", 0x0100, Plugin::Instrument, new PluginPixmapLoader("logo"), nullptr, nullptr};
}
...
void Xpressive::playNote(NotePlayHandle* nph, sampleFrame* working_buffer) {
	...
	if (nph->totalFramesPlayed() == 0 || nph->m_pluginData == nullptr) {
		...
		ExprFront* exprO2
			= new ExprFront(m_outputExpression[1].constData(), Engine::audioEngine()->processingSampleRate());
