...
public:
	...
	Oscillator(const IntModel* wave_shape_model, const IntModel* mod_algo_model, const float& freq,
		const float& detuning_div_samplerate, const float& phase_offset, const float& volume,
	...
	inline void setUseWaveTable(bool n) { m_useWaveTable = n; }
	...
	inline wtSampleControl getWtSampleControl(const float sample) const {
		...
		if (control.f1 < 0) { control.f1 += OscillatorConstants::WAVETABLE_LENGTH; }
		control.f2 = control.f1 < OscillatorConstants::WAVETABLE_LENGTH - 1 ? control.f1 + 1 : 0;
		control.band
			= waveTableBandFromFreq(m_freq * m_detuning_div_samplerate * Engine::audioEngine()->processingSampleRate());
		return control;
	}
	...
	static inline int waveTableBandFromFreq(float freq) {
		...
		int band = (69 + static_cast<int>(std::ceil(12.0f * std::log2(freq / 440.0f))))
			/ OscillatorConstants::SEMITONES_PER_TABLE;
		...
		return band <= 1 ? 1
			: band >= OscillatorConstants::WAVE_TABLES_PER_WAVEFORM_COUNT - 1
			? OscillatorConstants::WAVE_TABLES_PER_WAVEFORM_COUNT - 1
			: band;
	}
...
private:
	...
	/* Multiband WaveTable */
	static sample_t s_waveTables[WaveShapes::NumWaveShapeTables][OscillatorConstants::WAVE_TABLES_PER_WAVEFORM_COUNT]
								[OscillatorConstants::WAVETABLE_LENGTH];
