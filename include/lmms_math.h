...
static inline float safeDbfsToAmp(float dbfs) { return std::isinf(dbfs) ? 0.0f : std::pow(10.f, dbfs * 0.05f); }
...
//! @return Linear amplitude
static inline float dbfsToAmp(float dbfs) { return std::pow(10.f, dbfs * 0.05f); }
