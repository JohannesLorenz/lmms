...
bool InstrumentTrack::keyRangeImport() const { return m_microtuner.enabled() && m_microtuner.keyRangeImport(); }
	...
	case MidiNoteOn:
		if (event.velocity() > 0) {
			...
			if (m_notes[event.key()] == nullptr && event.key() >= firstKey() && event.key() <= lastKey()) {
				...
				if (!Engine::audioEngine()->addPlayHandle(nph)) { m_notes[event.key()] = nullptr; }
			}
			eventHandled = true;
			break;
		}
...
{
	if (m_instrument != nullptr) { return m_instrument->displayName(); }
	return QString();
}

void InstrumentTrack::deleteNotePluginData(NotePlayHandle* n)
{
	if (m_instrument != nullptr) { m_instrument->deleteNotePluginData(n); }
}
...
void InstrumentTrack::updateMixerChannel() { m_audioPort.setNextMixerChannel(m_mixerChannelModel.value()); }
...
bool InstrumentTrack::play(const TimePos& _start, const fpp_t _frames, const f_cnt_t _offset, int _clip_num)
{
	...
	for (clipVector::Iterator it = clips.begin(); it != clips.end(); ++it) {
		...
		if (_clip_num < 0) { cur_start -= c->startPosition(); }
...
void InstrumentTrack::loadTrackSpecificSettings(const QDomElement& thisElement)
{
	...
	if (!m_previewMode) { m_mixerChannelModel.loadSettings(thisElement, "mixch"); }
