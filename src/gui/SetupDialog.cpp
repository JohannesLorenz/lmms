...
SetupDialog::SetupDialog(ConfigTabs tab_to_open)
	...
	, m_trackDeletionWarning(ConfigManager::inst()->value("ui", "trackdeletionwarning", "1").toInt())
	...
	, m_hqAudioDev(ConfigManager::inst()->value("audioengine", "hqaudio").toInt())
	, m_bufferSize(ConfigManager::inst()->value("audioengine", "framesperaudiobuffer").toInt())
	...
	, m_backgroundPicFile(QDir::toNativeSeparators(ConfigManager::inst()->backgroundPicFile()))
{
	...
	addLedCheckBox(tr("Show warning when deleting tracks"), gui_tw, counter, m_trackDeletionWarning,
		SLOT(toggleTrackDeletionWarning(bool)), false);
...
void SetupDialog::accept()
{
	...
	ConfigManager::inst()->setValue("ui", "trackdeletionwarning", QString::number(m_trackDeletionWarning));
	...
	ConfigManager::inst()->setValue("audioengine", "audiodev", m_audioIfaceNames[m_audioInterfaces->currentText()]);
	ConfigManager::inst()->setValue("app", "nanhandler", QString::number(m_NaNHandler));
	ConfigManager::inst()->setValue("audioengine", "hqaudio", QString::number(m_hqAudioDev));
	ConfigManager::inst()->setValue("audioengine", "framesperaudiobuffer", QString::number(m_bufferSize));
	ConfigManager::inst()->setValue("audioengine", "mididev", m_midiIfaceNames[m_midiInterfaces->currentText()]);
...
void SetupDialog::toggleTrackDeletionWarning(bool enabled) { m_trackDeletionWarning = enabled; }
...
void SetupDialog::setBufferSize(int value)
{
	...
	m_bufferSizeLbl->setText(
		tr("Frames: %1\nLatency: %2 ms")
			.arg(m_bufferSize)
			.arg(1000.0f * m_bufferSize / Engine::audioEngine()->processingSampleRate(), 0, 'f', 1));
}
