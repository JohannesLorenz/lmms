	...
	m_legacySEPattern(ConfigManager::inst()->value("ui", "legacysebb", "0").toInt())
{
	...
	if (s_stepBtnOn200 == nullptr) { s_stepBtnOn200 = new QPixmap(embed::getIconPixmap("step_btn_on_200")); }

	if (s_stepBtnOff == nullptr) { s_stepBtnOff = new QPixmap(embed::getIconPixmap("step_btn_off")); }

	if (s_stepBtnOffLight == nullptr) { s_stepBtnOffLight = new QPixmap(embed::getIconPixmap("step_btn_off_light")); }
...
void MidiClipView::constructContextMenu(QMenu* _cm)
{
	...
	_cm->addAction(embed::getIconPixmap("edit_erase"), tr("Clear all notes"), m_clip, SLOT(clear()));
	_cm->addSeparator();
	...
	if (m_clip->type() == MidiClip::BeatClip) {
		...
		_cm->addAction(embed::getIconPixmap("step_btn_remove"), tr("Remove steps"), m_clip, SLOT(removeSteps()));
		_cm->addAction(embed::getIconPixmap("step_btn_duplicate"), tr("Clone Steps"), m_clip, SLOT(cloneSteps()));
	}
}
...
void MidiClipView::mousePressEvent(QMouseEvent* _me)
{
...

	{
		...
		float tmp = ((float(_me->x()) - BORDER_WIDTH) * float(m_clip->m_steps)) / float(width() - BORDER_WIDTH * 2);
...
void MidiClipView::wheelEvent(QWheelEvent* we)
{
	if (m_clip->m_clipType == MidiClip::BeatClip && (fixedClips() || pixelsPerBar() >= 96)
		...
		//	do calculations in floats to prevent rounding errors...
		float tmp
			= ((float(position(we).x()) - BORDER_WIDTH) * float(m_clip->m_steps)) / float(width() - BORDER_WIDTH * 2);
...
void MidiClipView::paintEvent(QPaintEvent*)
{
	...
	const int baseWidth = fixedClips() ? parentWidget()->width() - 2 * BORDER_WIDTH : width() - BORDER_WIDTH;
	...
	// beat clip paint event
		...
		QPixmap stepoff;
		QPixmap stepoffl;
		const int steps = qMax(1, m_clip->m_steps);
	...
	for (bar_t t = 1; t < m_clip->length().getBar(); ++t) {
		...
		p.drawLine(x_base + static_cast<int>(pixelsPerBar * t) - 1, rect().bottom() - (lineSize + BORDER_WIDTH),
			x_base + static_cast<int>(pixelsPerBar * t) - 1, rect().bottom() - BORDER_WIDTH);
	}
