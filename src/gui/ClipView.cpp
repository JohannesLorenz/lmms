	...
	, m_needsUpdate(true)
{
	...
	connect(m_clip, SIGNAL(lengthChanged()), this, SLOT(updateLength()));
	connect(getGUI()->songEditor()->m_editor->zoomingModel(), SIGNAL(dataChanged()), this, SLOT(updateLength()));
	connect(m_clip, SIGNAL(positionChanged()), this, SLOT(updatePosition()));
 ...
 */
bool ClipView::fixedClips() { return m_trackView->trackContainerView()->fixedClips(); }

// qproperty access functions, to be inherited & used by Clipviews
//! \brief CSS theming qproperty access method
QColor ClipView::mutedColor() const { return m_mutedColor; }

QColor ClipView::mutedBackgroundColor() const { return m_mutedBackgroundColor; }

QColor ClipView::selectedColor() const { return m_selectedColor; }

QColor ClipView::textColor() const { return m_textColor; }

QColor ClipView::textBackgroundColor() const { return m_textBackgroundColor; }

QColor ClipView::textShadowColor() const { return m_textShadowColor; }

QColor ClipView::patternClipBackground() const { return m_patternClipBackground; }

bool ClipView::gradient() const { return m_gradient; }

//! \brief CSS theming qproperty access method
void ClipView::setMutedColor(const QColor& c) { m_mutedColor = QColor(c); }

void ClipView::setMutedBackgroundColor(const QColor& c) { m_mutedBackgroundColor = QColor(c); }

void ClipView::setSelectedColor(const QColor& c) { m_selectedColor = QColor(c); }

void ClipView::setTextColor(const QColor& c) { m_textColor = QColor(c); }

void ClipView::setTextBackgroundColor(const QColor& c) { m_textBackgroundColor = c; }

void ClipView::setTextShadowColor(const QColor& c) { m_textShadowColor = QColor(c); }

void ClipView::setPatternClipBackground(const QColor& c) { m_patternClipBackground = QColor(c); }

void ClipView::setGradient(const bool& b) { m_gradient = b; }

// access needsUpdate member variable
bool ClipView::needsUpdate() { return m_needsUpdate; }
void ClipView::setNeedsUpdate(bool b) { m_needsUpdate = b; }
...
{
	if (fixedClips()) { setFixedWidth(parentWidget()->width()); }
	else
	{
		setFixedWidth(
...
void ClipView::randomizeColor() { setColor(&ColorChooser::getPalette(ColorChooser::Palette::Mixer)[rand() % 48]); }

void ClipView::resetColor() { setColor(nullptr); }
...
void ClipView::setColor(const QColor* color)
{
	...
	for (auto clipv : selectedClips)
	{
		...
		if (selectedClips.length() == 1) { clip->addJournalCheckPoint(); }
...
void ClipView::dragEnterEvent(QDragEnterEvent* dee)
{
	...
	if (tcw->canPasteSelection(clipPos, dee) == false) { dee->ignore(); }
	else
	{
		StringPairDrag::processDragEnterEvent(dee, "clip_" + QString::number(m_clip->getTrack()->type()));
	}
}
...
void ClipView::dropEvent(QDropEvent* de)
{
	...
	if (type != ("clip_" + QString::number(m_clip->getTrack()->type()))) { return; }
	...
	if (m_trackView->trackContainerView()->allowRubberband() == true)
	{
		...
		if (tcw->pasteSelection(clipPos, de) == true) { de->accept(); }
		return;
	}

	// Don't allow pasting a clip into itself.
	QObject* qwSource = de->source();
	if (qwSource != nullptr && dynamic_cast<ClipView*>(qwSource) == this) { return; }
 ...
 */
DataFile ClipView::createClipDataFiles(const QVector<ClipView*>& clipViews) const
{
	...
	for (clipViewVector::const_iterator it = clipViews.begin(); it != clipViews.end(); ++it)
	{
...
void ClipView::paintTextLabel(QString const& text, QPainter& painter)
{
	...
	if (elidedClipName.length() < 2) { elidedClipName = text.trimmed(); }
...
void ClipView::mousePressEvent(QMouseEvent* me)
{
	...
	auto active = me->button() == Qt::LeftButton ? QVector<ClipView*>() : getClickedClips();
	...
	if (!fixedClips() && me->button() == Qt::LeftButton)
	{
		...
		else
		{
			...
			else
			{
				...
				if (m_action == Move || m_action == Resize || m_action == ResizeLeft) { m_clip->setJournalling(false); }
				...
				if (m_action == Move)
				{
					...
					s_textFloat->setText(QString("%1:%2")
											 ...
											 .arg(m_clip->startPosition().getTicks() % TimePos::ticksPerBar()));
				}
				else if (m_action == Resize || m_action == ResizeLeft)
				{
					...
					s_textFloat->setText(tr("%1:%2 (%3:%4 to %5:%6)")
											 ...
											 .arg(m_clip->length().getTicks() % TimePos::ticksPerBar())
											 .arg(m_clip->startPosition().getBar() + 1)
											 .arg(m_clip->startPosition().getTicks() % TimePos::ticksPerBar())
											 .arg(m_clip->endPosition().getBar() + 1)
											 .arg(m_clip->endPosition().getTicks() % TimePos::ticksPerBar()));
				}
...
void ClipView::mouseMoveEvent(QMouseEvent* me)
{
		...
		if (mouseMovedDistance(me, 2) == true)
		{
			...
			if (m_action == CopySelection)
			{
				...
				for (auto it = so.begin(); it != so.end(); ++it)
				{
					...
					if (clipv != nullptr) { clipViews.push_back(clipv); }
				}
			}
			else
			{
			...
			new StringPairDrag(
				QString("clip_%1").arg(m_clip->getTrack()->type()), dataFile.toString(), thumbnail, this);
		}
	}
	...
	else if (m_action == MoveSelection)
	{
		...
		for (QVector<Clip*>::iterator it = clips.begin(); it != clips.end(); ++it)
		{
...
void ClipView::mouseReleaseEvent(QMouseEvent* me)
{
	...
	else if (m_action == Split)
	{
		...
		splitClip(unquantizedModHeld(me) ? relPos : quantizeSplitPos(relPos, me->modifiers() & Qt::ShiftModifier));
	}
...
void ClipView::contextMenuEvent(QContextMenuEvent* cme)
{
	...
	if (fixedClips() == false)
	{
		contextMenu.addAction(embed::getIconPixmap("cancel"),
			individualClip ? tr("Delete (middle mousebutton)") : tr("Delete selection (middle mousebutton)"),
			[this]() { contextMenuAction(Remove); });

		contextMenu.addSeparator();

		contextMenu.addAction(embed::getIconPixmap("edit_cut"), individualClip ? tr("Cut") : tr("Cut selection"),
	...
	contextMenu.addAction(embed::getIconPixmap("edit_copy"), individualClip ? tr("Copy") : tr("Copy selection"),
	...
	contextMenu.addAction(embed::getIconPixmap("muted"),
		(individualClip ? tr("Mute/unmute (<%1> + middle click)") : tr("Mute/unmute selection (<%1> + middle click)"))
			.arg(UI_CTRL_KEY),
...
void ClipView::contextMenuAction(ContextMenuAction action)
{
	...
	case Merge: mergeClips(active); break;
	}
}
...
QVector<ClipView*> ClipView::getClickedClips()
{
	...
	for (auto so : sos)
	{
		...
		if (clipv != nullptr) { selection.append(clipv); }
	}

	// If we clicked part of the selection, affect all selected clips. Otherwise affect the clip we clicked
	return selection.contains(this) ? selection : QVector<ClipView*>(1, this);
}
...
void ClipView::copy(QVector<ClipView*> clipvs)
{
	...
	copyStringPair(QString("clip_%1").arg(m_clip->getTrack()->type()), dataFile.toString());
}
...
void ClipView::mergeClips(QVector<ClipView*> clipvs)
{
	...
	InstrumentTrack* track = dynamic_cast<InstrumentTrack*>(clipvs.at(0)->getTrackView()->getTrack());
	...
	const auto earliestClipV = std::min_element(clipvs.constBegin(), clipvs.constEnd(),
		[](ClipView* a, ClipView* b) { return a->getClip()->startPosition() < b->getClip()->startPosition(); });
 ...
 */
float ClipView::pixelsPerBar() { return m_trackView->trackContainerView()->pixelsPerBar(); }
...
QColor ClipView::getColorForDisplay(QColor defaultColor)
{
	...
	auto clipColor = m_clip->hasColor() ? m_clip->usesCustomClipColor() ? m_clip->color() : m_clip->getTrack()->color()
										: defaultColor;
	...
	if (isSelected())
	{
		c = m_clip->hasColor() ? (muted ? mutedCustomColor.darker(350) : clipColor.darker(150)) : selectedColor();
	}
	else
	{
		if (muted) { c = m_clip->hasColor() ? mutedCustomColor.darker(250) : mutedBackgroundColor(); }
		else
		{
			c = clipColor;
		}
