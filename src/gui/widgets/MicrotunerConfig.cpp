...
MicrotunerConfig::MicrotunerConfig()
	...
	m_scaleNameEdit->setToolTip(
		tr("Scale description. Cannot start with \"!\" and cannot contain a newline character."));
	...
	m_scaleTextEdit->setPlainText(
		"100.0\n200.0\n300.0\n400.0\n500.0\n600.0\n700.0\n800.0\n900.0\n1000.0\n1100.0\n1200.0");
	m_scaleTextEdit->setToolTip(
		tr("Enter intervals on separate lines. Numbers containing a decimal point are treated as cents.\nOther inputs "
		   "are treated as integer ratios and must be in the form of \'a/b\' or \'a\'.\nUnity (0.0 cents or ratio 1/1) "
		   "is always present as a hidden first value; do not enter it manually."));
	...
	m_keymapNameEdit->setToolTip(
		tr("Keymap description. Cannot start with \"!\" and cannot contain a newline character."));
	...
	m_keymapTextEdit->setToolTip(
		tr("Enter key mappings on separate lines. Each line assigns a scale degree to a MIDI key,\nstarting with the "
		   "middle key and continuing in sequence.\nThe pattern repeats for keys outside of the explicit keymap "
		   "range.\nMultiple keys can be mapped to the same scale degree.\nEnter \'x\' if you wish to leave the key "
		   "disabled / not mapped."));
...
bool MicrotunerConfig::applyKeymap() {
	...
	auto newKeymap = std::make_shared<Keymap>(m_keymapNameEdit->text(), std::move(newMap), m_firstKeyModel.value(),
