	...
	, SerializingObjectHook()
{
	...
	channelArea->setFixedHeight(mixerLineSize.height() + style()->pixelMetric(QStyle::PM_ScrollBarExtent));
	...
	connect(getGUI()->mainWindow(), SIGNAL(periodicUpdate()), this, SLOT(updateFaders()));
...
void MixerView::saveSettings(QDomDocument& _doc, QDomElement& _this) { MainWindow::saveWidgetState(this, _this); }
...
MixerView::MixerChannelView::MixerChannelView(QWidget* _parent, MixerView* _mv, int channelIndex)
{
	...
	m_fader = new Fader(&mixerChannel->m_volumeModel, tr("Fader %1").arg(channelIndex), m_mixerLine);
	m_fader->setLevelsDisplayedInDBFS();
	m_fader->setMinPeak(dbfsToAmp(-42));
	m_fader->setMaxPeak(dbfsToAmp(9));

	m_fader->move(16 - m_fader->width() / 2, m_mixerLine->height() - m_fader->height() - 5);
	...
	connect(&mixerChannel->m_soloModel, SIGNAL(dataChanged()), _mv, SLOT(toggledSolo()), Qt::DirectConnection);
...
void MixerView::deleteChannel(int index)
{
	...
	if (selLine >= m_mixerChannelViews.size()) { selLine = m_mixerChannelViews.size() - 1; }
...
void MixerView::moveChannelLeft(int index) { moveChannelLeft(index, index - 1); }

void MixerView::moveChannelRight(int index) { moveChannelLeft(index + 1, index + 1); }
...
void MixerView::keyPressEvent(QKeyEvent* e)
{
	...
	case Qt::Key_Enter:
	case Qt::Key_Return:
	case Qt::Key_F2: renameChannel(m_currentMixerLine->channelIndex()); break;
	}
}
