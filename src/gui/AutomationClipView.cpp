	...
	, m_paintPixmap()
{
	...
	if (s_clip_rec == nullptr) { s_clip_rec = new QPixmap(embed::getIconPixmap("clip_rec")); }
...
void AutomationClipView::disconnectObject(QAction* _a)
{
	...
	if (j && dynamic_cast<AutomatableModel*>(j))
	{
		...
		m_clip->m_objects.erase(
			std::find(m_clip->m_objects.begin(), m_clip->m_objects.end(), dynamic_cast<AutomatableModel*>(j)));
		update();
...
void AutomationClipView::constructContextMenu(QMenu* _cm)
{
	...
	_cm->addAction(embed::getIconPixmap("edit_erase"), tr("Clear"), m_clip, SLOT(clear()));
	_cm->addSeparator();
...
void AutomationClipView::paintEvent(QPaintEvent*)
{
	...
	const float ppb = fixedClips()
		? (parentWidget()->width() - 2 * BORDER_WIDTH) / (float)m_clip->timeMapLength().getBar()
		: pixelsPerBar();
	...
	if (m_clip->isRecording()) { p.drawPixmap(1, rect().bottom() - s_clip_rec->height(), *s_clip_rec); }
