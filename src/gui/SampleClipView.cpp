...
void SampleClipView::updateSample()
{
	...
	ToolTip::add(this,
		(m_clip->m_sampleBuffer->audioFile() != "") ? PathUtil::toAbsolute(m_clip->m_sampleBuffer->audioFile())
...
void SampleClipView::paintEvent(QPaintEvent* pe)
{
	...
	auto clipColor = m_clip->hasColor()
		? (m_clip->usesCustomClipColor() ? m_clip->color() : m_clip->getTrack()->color())
	...
	if (selected) { p.setPen(p.pen().brush().color().darker(150)); }

	const int spacing = BORDER_WIDTH + 1;
	const float ppb = fixedClips() ? (parentWidget()->width() - 2 * BORDER_WIDTH) / (float)m_clip->length().getBar()
								   : pixelsPerBar();
	...
	QRect r = QRect(offset, spacing, qMax(static_cast<int>(m_clip->sampleLength() * ppb / ticksPerBar), 1),
