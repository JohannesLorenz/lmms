...
const QVector<float> AutomationEditor::m_zoomXLevels = {0.125f, 0.25f, 0.5f, 1.0f, 2.0f, 4.0f, 8.0f};
	...
	, m_backgroundShade(0, 0, 0)
{
	...
	// add time-line
	m_timeLine
		= new TimeLineWidget(VALUES_WIDTH, 0, m_ppb, Engine::getSong()->getPlayPos(Song::Mode_PlayAutomationClip),
			m_currentPosition, Song::Mode_PlayAutomationClip, this);
...
void AutomationEditor::setCurrentClip(AutomationClip* new_clip)
{
	...
	if (m_clip != nullptr) { connect(m_clip, SIGNAL(dataChanged()), this, SLOT(update())); }
...
void AutomationEditor::drawLine(int x0In, float y0, int x1In, float y1)
{
	...
	if (deltax < AutomationClip::quantization()) { return; }

	deltax /= AutomationClip::quantization();

	float yscale = deltay / (deltax);

	if (x0 < x1) { xstep = AutomationClip::quantization(); }
	else
	{
...
bool AutomationEditor::fineTuneValue(timeMap::iterator node, bool editingOutValue)
{
	...
	double value = QInputDialog::getDouble(this, tr("Edit Value"),
		...
		m_clip->firstObject()->minValue<float>(), m_clip->firstObject()->maxValue<float>(), 3, &ok);
		...
		case DRAW_OUTVALUES: {
			...
			if (m_mouseDownLeft)
			{
				...
				else // If we didn't click an outValue
				{
					...
					TimePos quantizedPos = Note::quantized(TimePos(posTicks), m_clip->quantization());
		...
		case DRAW: {
			...
			if (m_mouseDownLeft)
			{
				if (m_action == MOVE_VALUE)
				{
					...
					m_clip->setDragValue(TimePos(posTicks), level, true, mouseEvent->modifiers() & Qt::ControlModifier);
...
void AutomationEditor::resizeEvent(QResizeEvent* re)
{
	...
	if (Engine::getSong())
	{
		Engine::getSong()->getPlayPos(Song::Mode_PlayAutomationClip).m_timeLine->setFixedWidth(width());
	}
...
{
	if (!validClip()) { return; }
	if (m_clip->getTrack() && inPatternEditor()) { Engine::patternStore()->stop(); }
	else
	{
		Engine::getSong()->stop();
	}
