...
const QVector<float> SongEditor::m_zoomLevels = {0.125f, 0.25f, 0.5f, 1.0f, 2.0f, 4.0f, 8.0f, 16.0f};
...
SongEditor::SongEditor(Song* song)
	...
	connect(this, SIGNAL(zoomingValueChanged(float)), m_positionLine, SLOT(zoomChange(float)));
...
void SongEditor::setHighQuality(bool hq) {
	Engine::audioEngine()->changeQuality(AudioEngine::qualitySettings(
		hq ? AudioEngine::qualitySettings::Mode_HighQuality : AudioEngine::qualitySettings::Mode_Draft));
}
		...
		for (auto& it : findChildren<selectableObject*>()) {
			...
			if (clip) {
				...
				bool isBeetweenRubberbandTimePos
					= clip->getClip()->endPosition() >= qMin(m_rubberbandStartTimePos, rubberbandTimePos)
...
SongEditorWindow::SongEditorWindow(Song* song)
	...
	m_addPatternTrackAction = new QAction(embed::getIconPixmap("add_pattern_track"), tr("Add pattern-track"), this);
...
void SongEditorWindow::adjustUiAfterProjectLoad() {
	...
	getGUI()->mainWindow()->workspace()->setActiveSubWindow(qobject_cast<QMdiSubWindow*>(parentWidget()));
