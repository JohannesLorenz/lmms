...
const QVector<float> PianoRoll::m_zoomLevels = {0.125f, 0.25f, 0.5f, 1.0f, 1.5f, 2.0f, 4.0f, 8.0f};

const QVector<float> PianoRoll::m_zoomYLevels = {0.25f, 0.5f, 1.0f, 1.5f, 2.0f, 2.5f, 3.0f, 4.0f};
	...
	, m_blackKeyWidth(BLACK_KEY_WIDTH)
{
	...
	if (s_toolSelect == nullptr) { s_toolSelect = new QPixmap(embed::getIconPixmap("edit_select")); }
	if (s_toolMove == nullptr) { s_toolMove = new QPixmap(embed::getIconPixmap("edit_move")); }
	if (s_toolOpen == nullptr) { s_toolOpen = new QPixmap(embed::getIconPixmap("automation")); }
	...
	// add time-line
	m_timeLine = new TimeLineWidget(m_whiteKeyWidth, 0, m_ppb, Engine::getSong()->getPlayPos(Song::Mode_PlayMidiClip),
		m_currentPosition, Song::Mode_PlayMidiClip, this);
...
void PianoRoll::setCurrentMidiClip(MidiClip* newMidiClip)
{
	...
	if (Engine::getSong()->isPlaying() && Engine::getSong()->playMode() == Song::Mode_PlayMidiClip)
	{
	...
	connect(m_midiClip->instrumentTrack()->microtuner()->keyRangeImportModel(), SIGNAL(dataChanged()), this,
		SLOT(update()));
...
{
	if (m_midiClip == clip) { setCurrentMidiClip(nullptr); }
}
...
void PianoRoll::drawDetuningInfo(QPainter& _p, const Note* _n, int _x, int _y) const
{
	...
	for (timeMap::const_iterator it = map.begin(); it != map.end(); ++it)
	{
		...
		else
		{
			...
			case AutomationClip::LinearProgression: _p.drawLine(old_x, pre_y, cur_x, cur_y); break;
			}
...
void PianoRoll::keyPressEvent(QKeyEvent* ke)
{
	...
	case Qt::Key_Left: {
		...
		if (ke->modifiers() & Qt::ControlModifier && m_action == ActionNone)
		{
			...
			if (hasValidMidiClip()) { shiftPos(direction * TimePos::ticksPerBar()); }
		}
		else if (ke->modifiers() & Qt::ShiftModifier && m_action == ActionNone)
		{
			// move notes
		...
		else if (ke->modifiers() & Qt::AltModifier)
		{
			...
			if (hasValidMidiClip())
			{
				MidiClip* c = direction > 0 ? m_midiClip->nextMidiClip() : m_midiClip->previousMidiClip();
				if (c != nullptr) { setCurrentMidiClip(c); }
			}
		}
		else
		{
...
void PianoRoll::keyReleaseEvent(QKeyEvent* ke)
{
	...
	case Qt::Key_Z:
	case Qt::Key_R:
		if (hasValidMidiClip() && ke->modifiers() == Qt::ControlModifier) { update(); }
		break;
	}
...
void PianoRoll::mousePressEvent(QMouseEvent* me)
{
	...
	if (m_editMode == ModeEditDetuning && noteUnderMouse())
	{
		...
		if (detuningClip.data() != nullptr) { detuningClip->disconnect(this); }
	...
	if (me->y() > PR_TOP_MARGIN)
	{
		...
		if (x > m_whiteKeyWidth)
		{
			...
			else if (me->button() == Qt::LeftButton && m_editMode == ModeDraw)
			{
				...
				{
					if (!created_new_note) { m_midiClip->addJournalCheckPoint(); }
...
void PianoRoll::mouseReleaseEvent(QMouseEvent* me)
{
	...
	if (hasValidMidiClip())
	{
		...
		m_midiClip->instrumentTrack()->pianoModel()->handleKeyRelease(m_lastKey);
		pauseChordNotes(m_lastKey);
	}
			...
			for (int i = 0; i < notes.size(); ++i)
			{
					...
					|| (isUnderPosition && n->selected() && altPressed))
				{
					if (m_noteEditMode == NoteEditVolume)
					{
						...
						m_midiClip->instrumentTrack()->processInEvent(
...
void PianoRoll::resizeEvent(QResizeEvent* re)
{
	updatePositionLineHeight();
	updateScrollbars();
	Engine::getSong()->getPlayPos(Song::Mode_PlayMidiClip).m_timeLine->setFixedWidth(width());
	update();
}
...
void PianoRoll::focusOutEvent(QFocusEvent*)
{
	if (hasValidMidiClip())
	{
		...
		{
			m_midiClip->instrumentTrack()->pianoModel()->midiEventProcessor()->processInEvent(
...
{
	if (m_midiClip->getTrack()->trackContainer() == Engine::patternStore()) { return Song::Mode_PlayPattern; }
...
void PianoRoll::play()
{
	...
	if (Engine::getSong()->playMode() != Song::Mode_PlayMidiClip) { Engine::getSong()->playMidiClip(m_midiClip); }
	else
	{
...
void PianoRoll::record()
{
	...
	if (m_recording || !hasValidMidiClip()) { return; }
...
void PianoRoll::recordAccompany()
{
	...
	if (m_midiClip->getTrack()->trackContainer() == Engine::getSong()) { Engine::getSong()->playSong(); }
	else
	{
...
bool PianoRoll::toggleStepRecording()
{
	...
	else
	{
		if (hasValidMidiClip())
		{
			...
			{
				m_stepRecorder.start(Engine::getSong()->getPlayPos(Song::Mode_PlayMidiClip), newNoteLen());
			}
		}
	}
...
void PianoRoll::startRecordNote(const Note& n)
{
	if (hasValidMidiClip())
	{
		...
		{
			TimePos sub;
			if (Engine::getSong()->playMode() == Song::Mode_PlaySong) { sub = m_midiClip->startPosition(); }
...
void PianoRoll::updateYScroll()
{
	...
	if (m_startKey > m_totalKeysToScroll) { m_startKey = m_totalKeysToScroll; }
	m_topBottomScroll->setValue(m_totalKeysToScroll - m_startKey);
}
...
void PianoRoll::pasteNotes()
{
	...
	if (!value.isEmpty())
	{
		...
		if (!list.isEmpty()) { m_midiClip->addJournalCheckPoint(); }
...
void PianoRoll::updatePositionAccompany(const TimePos& t)
{
	...
	if (m_recording && hasValidMidiClip() && s->playMode() != Song::Mode_PlayMidiClip)
	{
		TimePos pos = t;
		if (s->playMode() != Song::Mode_PlayPattern) { pos -= m_midiClip->startPosition(); }
		if ((int)pos > 0)
		{
...
bool PianoRoll::mouseOverNote() { return hasValidMidiClip() && noteUnderMouse() != nullptr; }
	...
	, m_editor(new PianoRoll())
{
	...
	m_recordAccompanyAction->setToolTip(
		tr("Record notes from MIDI-device/channel-piano while playing song or pattern track"));
	...
	// Import / export
	QAction* importAction = new QAction(embed::getIconPixmap("project_import"), tr("Import clip"), m_fileToolsButton);
...
const MidiClip* PianoRollWindow::currentMidiClip() const { return m_editor->currentMidiClip(); }

void PianoRollWindow::setGhostMidiClip(MidiClip* clip) { m_editor->setGhostMidiClip(clip); }
...
void PianoRollWindow::ghostClipSet(bool state) { m_clearGhostButton->setEnabled(state); }

void PianoRollWindow::exportMidiClip()
{
	FileDialog exportDialog(this, tr("Export clip"), "", tr("XML clip file (*.xpt *.xptz)"));
		...
		&& !exportDialog.selectedFiles().first().isEmpty())
	{
		...
		{
			TextFloat::displayMessage(tr("Export clip success"), tr("Clip saved to %1").arg(fullPath),
...
void PianoRollWindow::importMidiClip()
{
	...
	if (!m_editor->m_midiClip->empty()
		&& QMessageBox::warning(nullptr, tr("Import clip."),
	...
	FileDialog importDialog(this, tr("Open clip"), "", tr("XML clip file (*.xpt *.xptz)"));
	...
	if (importDialog.exec() == QDialog::Accepted && !importDialog.selectedFiles().isEmpty())
	{
		...
		TextFloat::displayMessage(tr("Import clip success"), tr("Imported clip %1!").arg(fullPath),
