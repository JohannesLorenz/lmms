...
DataFile::typeDescStruct DataFile::s_types[DataFile::TypeCount] = {{DataFile::UnknownType, "unknown"},
	...
	{DataFile::EffectSettings, "effectsettings"}, {DataFile::MidiClip, "midiclip"}};
	...
	if (!elementsByTagName("song").item(0).isNull()) {
		...
		for (int i = 0; !tracks.item(i).isNull(); i++) {
			...
			instrument.parentNode().toElement().setAttribute(
				"basenote", instrument.parentNode().toElement().attribute("basenote").toInt() + 12);
		...
		QDomElement instrument = instruments.item(0).toElement();
		if (affected(instrument)) { preset.setAttribute("basenote", preset.attribute("basenote").toInt() + 12); }
	}
}
