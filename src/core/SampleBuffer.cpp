...
sample_rate_t SampleBuffer::audioEngineSampleRate() { return Engine::audioEngine()->processingSampleRate(); }
	...
	if (fileLoadError)
	{
		...
		QString message = tr("Audio files are limited to %1 MB "
							 "in size and %2 minutes of playing time")
							  .arg(fileSizeMax)
							  .arg(sampleLengthMax);
		if (getGUI() != nullptr) { QMessageBox::information(nullptr, title, message, QMessageBox::Ok); }
		else
		{
	...
	sampleFrame* ab, handleState* state, const fpp_t frames, const float freq, const LoopMode loopMode)
{
	...
	const double freqFactor
		= (double)freq / (double)m_frequency * m_sampleRate / Engine::audioEngine()->processingSampleRate();
...
QString& SampleBuffer::toBase64(QString& dst) const
{
	...
	FLAC__stream_encoder_set_sample_rate(flacEnc, Engine::audioEngine()->sampleRate());
		...
		for (f_cnt_t f = 0; f < remaining; ++f)
		{
			for (ch_cnt_t ch = 0; ch < DEFAULT_CHANNELS; ++ch)
			{
				buf[f * DEFAULT_CHANNELS + ch]
					= (FLAC__int32)(AudioEngine::clip(m_data[f + frameCnt][ch]) * OUTPUT_SAMPLE_MULTIPLIER);
			}
		}
