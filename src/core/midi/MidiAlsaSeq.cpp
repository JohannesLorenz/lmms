...
void MidiAlsaSeq::processOutEvent(const MidiEvent& event, const TimePos& time, const MidiPort* port)
{
	...
	case MidiKeyPressure: snd_seq_ev_set_keypress(&ev, event.channel(), event.key(), event.velocity()); break;
	...
	while (m_quit == false)
	{
		...
		while (snd_seq_event_input_pending(m_seqHandle, true) > 0)
		{
			...
			case SND_SEQ_EVENT_NOTEOFF:
				dest->processInEvent(
					MidiEvent(MidiNoteOff, ev->data.note.channel, ev->data.note.note, ev->data.note.velocity, source),
					TimePos(ev->time.tick));
				break;

			case SND_SEQ_EVENT_KEYPRESS:
				dest->processInEvent(MidiEvent(MidiKeyPressure, ev->data.note.channel, ev->data.note.note,
