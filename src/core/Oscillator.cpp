...
Oscillator::Oscillator(const IntModel* wave_shape_model, const IntModel* mod_algo_model, const float& freq,
	const float& detuning_div_samplerate, const float& phase_offset, const float& volume, Oscillator* sub_osc)
...
void Oscillator::update(sampleFrame* ab, const fpp_t frames, const ch_cnt_t chnl, bool modulator)
{
	...
	if (m_subOsc != nullptr)
	{
		...
		{
		case PhaseModulation: updatePM(ab, frames, chnl); break;
		case AmplitudeModulation: updateAM(ab, frames, chnl); break;
		case SignalMix: updateMix(ab, frames, chnl); break;
		case SynchronizedBySubOsc: updateSync(ab, frames, chnl); break;
		case FrequencyModulation: updateFM(ab, frames, chnl);
		}
	}
	else
	{
...
void Oscillator::generateFromFFT(int bands, sample_t* table)
{
	...
	normalize(
		s_sampleBuffer, table, OscillatorConstants::WAVETABLE_LENGTH, 2 * OscillatorConstants::WAVETABLE_LENGTH + 1);
}
...
void Oscillator::generateAntiAliasUserWaveTable(SampleBuffer* sampleBuffer)
{
	...
	for (int i = 0; i < OscillatorConstants::WAVE_TABLES_PER_WAVEFORM_COUNT; ++i)
	{
		...
		Oscillator::generateFromFFT(OscillatorConstants::MAX_FREQ / freqFromWaveTableBand(i),
			(*(sampleBuffer->m_userAntiAliasWaveTable))[i].data());
	}
}

sample_t Oscillator::s_waveTables[Oscillator::WaveShapes::NumWaveShapeTables]
...
{
	Oscillator::s_specBuf
		= (fftwf_complex*)fftwf_malloc((OscillatorConstants::WAVETABLE_LENGTH * 2 + 1) * sizeof(fftwf_complex));
	Oscillator::s_fftPlan
		= fftwf_plan_dft_r2c_1d(OscillatorConstants::WAVETABLE_LENGTH, s_sampleBuffer, s_specBuf, FFTW_MEASURE);
	Oscillator::s_ifftPlan
		= fftwf_plan_dft_c2r_1d(OscillatorConstants::WAVETABLE_LENGTH, s_specBuf, s_sampleBuffer, FFTW_MEASURE);
...
void Oscillator::generateWaveTables()
{
	...
	auto simpleGen = [](WaveShapes shape, generator_t generator) {
		...
		std::fill(std::begin(s_waveTables[shapeID][OscillatorConstants::WAVE_TABLES_PER_WAVEFORM_COUNT - 1]),
			std::end(s_waveTables[shapeID][OscillatorConstants::WAVE_TABLES_PER_WAVEFORM_COUNT - 1]), 0.f);
		...
		for (int i = OscillatorConstants::WAVE_TABLES_PER_WAVEFORM_COUNT - 1; i >= 0; i--)
		{
			...
			if (i)
			{
				std::copy(s_waveTables[shapeID][i], s_waveTables[shapeID][i] + OscillatorConstants::WAVETABLE_LENGTH,
	...
	auto fftGen = []() {
		...
		for (int i = 0; i < OscillatorConstants::WAVE_TABLES_PER_WAVEFORM_COUNT; ++i)
		{
			...
			fftwf_execute(s_fftPlan);
			generateFromFFT(OscillatorConstants::MAX_FREQ / freqFromWaveTableBand(i),
				s_waveTables[WaveShapes::MoogSawWave - FirstWaveShapeTable][i]);
		}
		...
		for (int i = 0; i < OscillatorConstants::WAVE_TABLES_PER_WAVEFORM_COUNT; ++i)
		{
			...
			fftwf_execute(s_fftPlan);
			generateFromFFT(OscillatorConstants::MAX_FREQ / freqFromWaveTableBand(i),
				s_waveTables[WaveShapes::ExponentialWave - FirstWaveShapeTable][i]);
		}
	};
...
{
	if (m_subOsc != nullptr) { m_subOsc->update(_ab, _frames, _chnl); }
	recalcPhase();
...
template <> inline sample_t Oscillator::getSample<Oscillator::SineWave>(const float sample)
{
	const float current_freq = m_freq * m_detuning_div_samplerate * Engine::audioEngine()->processingSampleRate();

	if (!m_useWaveTable || current_freq < OscillatorConstants::MAX_FREQ) { return sinSample(sample); }
	else
	{
		return 0;
	}
...
{
	if (m_useWaveTable && !m_isModulator) { return wtSample(m_userWave->m_userAntiAliasWaveTable, _sample); }
	else
	{
