...
EnvelopeAndLfoParameters::EnvelopeAndLfoParameters(float _value_for_zero_amount, Model* _parent)
	...
	if (s_lfoInstances == nullptr) { s_lfoInstances = new LfoInstances(); }
	...
	connect(Engine::audioEngine(), SIGNAL(sampleRateChanged()), this, SLOT(updateSampleVars()));

	m_lfoShapeData = new sample_t[Engine::audioEngine()->framesPerPeriod()];
...
void EnvelopeAndLfoParameters::updateSampleVars() {
	...
	const float frames_per_lfo_oscillation = SECS_PER_LFO_OSCILLATION * Engine::audioEngine()->processingSampleRate();
