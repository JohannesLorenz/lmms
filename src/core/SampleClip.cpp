...
void SampleClip::changeLength(const TimePos& _length) { Clip::changeLength(qMax(static_cast<int>(_length), 1)); }
...
bool SampleClip::isPlaying() const { return m_isPlaying; }

void SampleClip::setIsPlaying(bool isPlaying) { m_isPlaying = isPlaying; }

void SampleClip::updateLength() { emit sampleChanged(); }

TimePos SampleClip::sampleLength() const { return (int)(m_sampleBuffer->frames() / Engine::framesPerTick()); }

void SampleClip::setSampleStartFrame(f_cnt_t startFrame) { m_sampleBuffer->setStartFrame(startFrame); }
...
ClipView* SampleClip::createView(TrackView* _tv) { return new SampleClipView(this, _tv); }
