...
NotePlayHandle::~NotePlayHandle()
{
	...
	if (m_pluginData != nullptr) { m_instrumentTrack->deleteNotePluginData(this); }

	if (m_instrumentTrack->m_notes[key()] == this) { m_instrumentTrack->m_notes[key()] = nullptr; }
...
void NotePlayHandle::play(sampleFrame* _working_buffer)
{
	...
	if (key() < m_instrumentTrack->m_firstKeyModel.value() || key() > m_instrumentTrack->m_lastKeyModel.value())
	{
	...
	f_cnt_t framesThisPeriod = m_totalFramesPlayed == 0 ? Engine::audioEngine()->framesPerPeriod() - offset()
...
{
	if (instrumentTrack()->isSustainPedalPressed()) { return 4 * Engine::audioEngine()->framesPerPeriod(); }
