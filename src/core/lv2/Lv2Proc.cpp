...
void Lv2Proc::initPlugin()
{
	...
	m_instance = lilv_plugin_instantiate(
		m_plugin, Engine::audioEngine()->processingSampleRate(), m_features.featurePointers());
...
void Lv2Proc::createPort(std::size_t portNum)
{
	...
	case Lv2Ports::Type::Audio: {
		Lv2Ports::Audio* audio = new Lv2Ports::Audio(
			static_cast<std::size_t>(Engine::audioEngine()->framesPerPeriod()), portIsSideChain(m_plugin, lilvPort));
		port = audio;
		break;
	}
