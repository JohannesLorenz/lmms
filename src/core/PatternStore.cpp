...
PatternStore::PatternStore()
	...
	connect(&m_patternComboBoxModel, SIGNAL(dataUnchanged()), this, SLOT(currentPatternChanged()));
...
bool PatternStore::play(TimePos start, fpp_t frames, f_cnt_t offset, int clipNum) {
	...
	TrackList tl = tracks();
	for (Track* t : tl) {
		if (t->play(start, frames, offset, clipNum)) { notePlayed = true; }
	}
...
bar_t PatternStore::lengthOfPattern(int pattern) const {
	...
	for (Track* t : tl) {
		...
		if (pattern < t->numOfClips()) { maxLength = qMax(maxLength, t->getClip(pattern)->length()); }
	}
...
void PatternStore::currentPatternChanged() {
	...
	for (Track* t : tl) {
		if (t->type() == Track::PatternTrack) { t->dataChanged(); }
	}
}
