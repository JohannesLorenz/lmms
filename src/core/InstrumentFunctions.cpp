	...
	if ((_n->origin() == NotePlayHandle::OriginArpeggio
		...
		for (int octave_cnt = 0; octave_cnt < m_chordRangeModel.value(); ++octave_cnt) {
			...
			for (int i = 0; i < chord_table[selected_chord].size(); ++i) {
				...
				// different
				Engine::audioEngine()->addPlayHandle(NotePlayHandleManager::acquire(_n->instrumentTrack(), _n->offset(),
...
void InstrumentFunctionArpeggio::processNote(NotePlayHandle* _n) {
	...
	const f_cnt_t arp_frames
		= (f_cnt_t)(m_arpTimeModel.value() / 1000.0f * Engine::audioEngine()->processingSampleRate());
	...
	while (frames_processed < Engine::audioEngine()->framesPerPeriod()) {
		...
		// range-checking
		if (sub_note_key >= NumKeys || sub_note_key < 0 || Engine::audioEngine()->criticalXRuns()) { continue; }

		// create new arp-note

		// create sub-note-play-handle, only ptr to note is different
		// and is_arp_note=true
		Engine::audioEngine()->addPlayHandle(NotePlayHandleManager::acquire(_n->instrumentTrack(), frames_processed,
