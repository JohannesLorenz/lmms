	...
	, m_oldAutomatedValues()
{
	...
	connect(Engine::audioEngine(), SIGNAL(sampleRateChanged()), this, SLOT(updateFramesPerTick()));
	...
	{
		m_scales[i] = std::make_shared<Scale>();
	}
	for (int i = 0; i < MaxKeymapCount; i++)
	{
		m_keymaps[i] = std::make_shared<Keymap>();
	}
}
...
void Song::masterVolumeChanged() { Engine::audioEngine()->setMasterGain(m_masterVolumeModel.value() / 100.0f); }
...
void Song::savePos()
{
	...
	if (tl != nullptr) { tl->savePos(m_playPos[m_playMode]); }
}
...
void Song::processAutomations(const TrackList& tracklist, TimePos timeStart, fpp_t)
{
	...
	{
		if (track->type() == Track::AutomationTrack) { track->getClipsInRange(clips, 0, timeStart); }
	}
...
AutomationClip* Song::tempoAutomationClip() { return AutomationClip::globalAutomationClip(&m_tempoModel); }
...
void Song::clearProject()
{
	...
	if (getGUI() != nullptr && getGUI()->patternEditor()) { getGUI()->patternEditor()->m_editor->clearAllTracks(); }
	if (getGUI() != nullptr && getGUI()->songEditor()) { getGUI()->songEditor()->m_editor->clearAllTracks(); }
	if (getGUI() != nullptr && getGUI()->mixerView()) { getGUI()->mixerView()->clear(); }
	...
	if (getGUI() != nullptr && getGUI()->automationEditor()) { getGUI()->automationEditor()->setCurrentClip(nullptr); }

	if (getGUI() != nullptr && getGUI()->pianoRoll()) { getGUI()->pianoRoll()->reset(); }
	...
	AutomationClip::globalAutomationClip(&m_masterVolumeModel)->clear();
	AutomationClip::globalAutomationClip(&m_masterPitchModel)->clear();

	Engine::audioEngine()->doneChangeInModel();

	if (getGUI() != nullptr && getGUI()->getProjectNotes()) { getGUI()->getProjectNotes()->clear(); }
...
void Song::loadProject(const QString& fileName)
{
	...
	if (hasErrors())
	{
		if (getGUI() != nullptr)
		{
			QMessageBox::warning(nullptr, tr("LMMS Error report"), errorSummary(), QMessageBox::Ok);
		}
		else
		{
...
void Song::exportProjectMidi(QString const& exportFileName) const
{
	...
	if (exf) { exf->tryExport(tracks, patternStoreTracks, getTempo(), m_masterPitchModel.value(), exportFileName); }
	else
	{
