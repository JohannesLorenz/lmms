...
Track* Track::create(TrackTypes tt, TrackContainer* tc) {
	...
	if (tc == Engine::patternStore() && t) { t->createClipsForPattern(Engine::patternStore()->numOfPatterns() - 1); }
...
Track* Track::create(const QDomElement& element, TrackContainer* tc) {
	...
	if (t != nullptr) { t->restoreState(element); }
 ...
 */
int Track::numOfClips() { return m_clips.size(); }
