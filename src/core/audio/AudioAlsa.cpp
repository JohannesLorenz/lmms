...
AudioAlsa::~AudioAlsa() {
	...
	if (m_hwParams != nullptr) { snd_pcm_hw_params_free(m_hwParams); }
	...
	while (quit == false) {
		...
		while (len) {
			if (outbuf_pos == 0) {
				...
				convertToS16(temp, frames, audioEngine()->masterGain(), outbuf, m_convertEndian);
			}
...
int AudioAlsa::setHWParams(const ch_cnt_t _channels, snd_pcm_access_t _access) {
	...
	if ((err = snd_pcm_hw_params_set_rate(m_handle, m_hwParams, sampleRate(), 0)) < 0) {
		if ((err = snd_pcm_hw_params_set_rate(m_handle, m_hwParams, audioEngine()->baseSampleRate(), 0)) < 0) {
