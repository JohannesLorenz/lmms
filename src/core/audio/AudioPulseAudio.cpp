...
static void context_state_callback(pa_context* c, void* userdata)
{
	...
	case PA_CONTEXT_READY: {
		...
		pa_stream_connect_playback(_this->m_s, nullptr, &buffer_attr, PA_STREAM_ADJUST_LATENCY,
			nullptr, // volume
			nullptr);
...
void AudioPulseAudio::streamWriteCallback(pa_stream* s, size_t length)
{
	...
	while (fd < length / 4 && m_quit == false) {
		...
		int bytes = convertToS16(temp, frames, audioEngine()->masterGain(), pcmbuf, m_convertEndian);
		if (bytes > 0) { pa_stream_write(m_s, pcmbuf, bytes, nullptr, 0, PA_SEEK_RELATIVE); }
		fd += frames;
	}
