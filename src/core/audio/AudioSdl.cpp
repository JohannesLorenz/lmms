...
#else
	m_convertedBufSize = audioEngine()->framesPerPeriod() * channels() * sizeof(int_sample_t);
	m_convertedBufPos = 0;
...
void AudioSdl::sdlAudioCallback(Uint8* _buf, int _len)
{
	...
	while (_len) {
		if (m_convertedBufPos == 0) {
			...
			convertToS16(
				m_outBuf, frames, audioEngine()->masterGain(), (int_sample_t*)m_convertedBuf, m_outConvertEndian);
		}
